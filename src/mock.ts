...
/// Mirror of selection::resolve_with_base: the CLI-inherited base (ADR 0046) seeds the chain as an
...
/// Mirror of the backend `Gate`: whether the declared project layer may activate (ADR 0045).
...
/// global layer, the workspace's own triple, and the mcp base/universe (ADR 0046).
...
const mockCommands: IpcHandlers = {
  ...
  // The CLI-inherited base for a Claude workspace, minus servers the hub already has (ADR 0046).
  mcp_inherited(args) {
