...
export type ProviderId = "claude" | "codex" | "antigravity" | "gemini";
...
/// the inherited set with `add`; `base: "inherit"` applies `add`/`remove` over it. See ADR 0045.
...
/// One project-trust decision (ADR 0045): a repository's versioned `[tools]` activates only after the
...
/// Where an effective item came from (ADR 0045). `inherited` flows down from a layer above,
...
/// Toggle one hub id in a layer while preserving what the layers above contribute (ADR 0045). Turning
...
export type Workspace = {
  ...
  /// a replacement with an empty `add` selects no MCP servers. See ADR 0045.
  mcp: Selection | null;
  ...
  cleaned: boolean;
  /// Existing branch survives worktree cleanup; absent on older boards.
